...
public:
    ...
    static int gcd (int a, int b){
        a=abs(a); b= abs(b);
        int min=0,count=0,buf=0,max=0;
        if(a<=b) {min =a; max=b;}
        else {min=b; max=a;}
        while (true){
            count++;
            if (a%count==0 && b%count==0)buf = count;
            if (min==count) break;
            if (count==max) break;
        }
        return buf;
    }
    bool operator ==(Rational rR){
        if (this->Rr==rR.Rr) return true;
        if (fabs(this->Rr-rR.Rr)<1e-15) return true;
        return false;
    }
    Rational operator +(Rational rR){
        Rational RR;
        double rRr =this->Rr+rR.Rr;
        RR.Rr=rRr;
        return RR;
    }
    Rational operator -(Rational rR){
